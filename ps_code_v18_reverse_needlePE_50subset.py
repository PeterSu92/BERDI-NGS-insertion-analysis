...
def filter_sample(f_name,pe_name,template,f_filt_seqs,r_filt_seqs):
        ...
        s1 = filter_pe_mismatch(f_seqs3[0:1000],pe_seqs3[0:1000],gen_copied_seq_function(f_res),f_filt_seqs[2]) #right now using the scar
        seqs = s1[0]
                ...
                if filt_seq not in str(s.seq): #if the scar isn't found on the forward read
                    ...
                    #print('search oligo is '+str(len(search_oligo))+' bases long')
                        ...
                        if f > 0: #if the quality of bases between the end of the aligned region and the start of the scar is good#
                            print ("Appended region is from "+str(bar3.span()[1])+ " to "+ str(bar4.span()[0]))
