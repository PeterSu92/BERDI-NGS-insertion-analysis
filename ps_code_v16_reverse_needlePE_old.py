...
def filter_sample(f_name,pe_name,template,f_filt_seqs,r_filt_seqs):
        ...
        seqs = filter_pe_mismatch(f_seqs3,pe_seqs3,gen_copied_seq_function(f_res))
...
def filter_pe_mismatch(f_seqs,pe_seqs,copied_func):
    ...
    for s in f_seqs:
            ...
            with open('temp_seq_PE.fa','w') as sh: #create temporary seq file
                    temp_f_seq = SeqRecord(Seq(copied),id = 'f_read',name = 'f_read')
        ...
        # make temp sequence file for alignment
            ...
            needle_cline = NeedleCommandline(asequence='temp_temp_PE.fa', bsequence='temp_seq_PE.fa', gapopen=gapopen,
                                             gapextend=gapextend, outfile='PE.needle')
            needle_cline()
            
            aln_data = list(AlignIO.parse(open('PE.needle'),"emboss"))
            bin_scores = [[42,251],[205,501],[446,751],[687,1001],[928,1251],[1100,1500]]
            if len(str(aln_data[0][1].seq)) < 50 bp:
                lo_cutoff = bin_scores[0][0]
                hi_cutoff = bin_scores[0][1]
            elif len(str(aln_data[0][1].seq)) >= 50 bp and len(str(aln_data[0][1].seq)) < 100 bp:
                lo_cutoff = bin_scores[1][0]
                hi_cutoff = bin_scores[1][1]
            elif len(str(aln_data[0][1].seq)) >= 100 bp and len(str(aln_data[0][1].seq)) < 150 bp:
                lo_cutoff = bin_scores[2][0]
                hi_cutoff = bin_scores[2][1]
            elif len(str(aln_data[0][1].seq)) >= 150 bp and len(str(aln_data[0][1].seq)) < 200 bp:
                lo_cutoff = bin_scores[3][0]
                hi_cutoff = bin_scores[3][1]
            elif len(str(aln_data[0][1].seq)) >= 200 bp and len(str(aln_data[0][1].seq)) < 250 bp:
                lo_cutoff = bin_scores[4][0]
                hi_cutoff = bin_scores[4][1]
            elif len(str(aln_data[0][1].seq)) >= 250 bp and len(str(aln_data[0][1].seq)) < 300 bp:
...
def insertion_chunks(final_seqs,template,reaction_number):
    '''
    ...
    seq_chunks = []
    #insert_site = 0
    # reverse_search = [1,5,9,13,2,6,10,14] #PCRs with a forward facing MBP primer searches reverse
    # forward_search =  [3,4,7,8,11,12,15,16] #PCRs with a forward facing ZFP primer searches forward
    #pos = 0
    ...
    for i in range(len(final_seqs)):
       ...
       #    insert_site = end_pos
            ...
            continue
       elif abs((bar.span()[1]-bar.span()[0])+1) < len(final_seqs[i].seq.lstrip('-').strip('-')):
            discarded_reads += 1
            continue
               # if num_chunks == 0:
...
#               break           
            ...
            span_length = abs(bar.span()[1]-bar.span()[0])
            seq_chunks.append(span_length)
            insertions.append(len(final_seqs[i].seq)-bar.span()[0])
            #num_chunks +=1
            continue
                #if insert_site != 0:
       # if num_chunks == 0:
    ...
    print (str(read_at_end)+ ' reads reached the end without a suitable insertion')  
...
def insertion_site_freq(final_seqs,template,reaction_number):
    ...
    chunky_dict = insertion_chunks(final_seqs,template, reaction_number)
